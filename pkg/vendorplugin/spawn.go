...
const engineObservationTimeout = 10 * time.Second
...
func BuildLaunch(ctx context.Context, r *Registry, req SpawnRequest, mode agentic.LaunchMode) (agentic.Plan, error) {
	...
	// generic dispatch is by capability, never by system/runtime/model identity.
	launch, err = agentic.PrepareLaunchRequest(binding.System, launch, mode)
	if err != nil {
		return agentic.Plan{}, fmt.Errorf("vendorplugin: runtime %s rejected the launch request before observation: %w", binding.ID, err)
	}
	...
	if requestedEngine != (plugin.Ref{}) && mode != agentic.LaunchModeDryRun {
		...
		r.mu.RLock()
		registered, ok := r.engineAdapters[resolvedEngine]
		r.mu.RUnlock()
		if !ok {
			return agentic.Plan{}, fmt.Errorf("%w: %w: engine %#v", ErrEngineObservationAdapterMissing, inferenceengine.ErrEngineContractMissing, resolvedEngine)
		}
		observationCtx, cancel := context.WithTimeout(ctx, engineObservationTimeout)
		_, observationErr := resolveEngineObservations(observationCtx, registered, EngineObservationQuery{
			Engine: resolvedEngine, Runtime: binding.ID, Model: model.ID, Profile: launch.Profile,
		})
		cancel()
		if observationErr != nil {
			return agentic.Plan{}, fmt.Errorf("vendorplugin: inference-engine observations refused before launch: %w", observationErr)
		}
	}
...
func checkLaunchFidelity(runtime Runtime, model Model, effort string, req SpawnRequest, launch agentic.LaunchRequest) error {
	...
	if req.Profile != "" && launch.Profile != req.Profile {
		return fmt.Errorf("%w: vendor %s changed the caller-selected profile; only an absent profile may be filled by the vendor",
			ErrVendorContract, runtime.VendorID)
	}
