...
type Registry struct {
	...
	engineAdapters map[plugin.Ref]registeredEngineObservationAdapter
}
...
// NewRegistryWithEngineObservationAdapters returns an empty registry with an
// immutable, declaration-snapshotted adapter table. The batch is atomic: if
// any adapter is invalid, no Registry is returned and none is installed.
func NewRegistryWithEngineObservationAdapters(systems *agentic.Registry, adapters ...EngineObservationAdapter) (*Registry, error) {
	registered := make(map[plugin.Ref]registeredEngineObservationAdapter, len(adapters))
	for index, adapter := range adapters {
		entry, err := validateEngineObservationAdapter(adapter)
		if err != nil {
			return nil, fmt.Errorf("vendorplugin: observation adapter %d: %w", index, err)
		}
		if _, duplicate := registered[entry.declaration.Engine]; duplicate {
			return nil, fmt.Errorf("%w: duplicate engine %#v", ErrEngineObservationAdapterInvalid, entry.declaration.Engine)
		}
		registered[entry.declaration.Engine] = entry
	}
	registry := NewRegistry(systems)
	registry.engineAdapters = registered
	return registry, nil
}
