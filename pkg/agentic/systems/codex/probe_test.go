...
// The version grammar, pinned against the real capture first: `codex
// --version` prints `codex-cli <triple>` on stdout, exit 0 (installed
// 0.153.4). Helper-direct and disclosed as a bound — parseToolRelease
// is the probe's pure half; the rows below drive the production prober
// through agentic.ProbeToolRelease against fake binaries, and never
// execute a real provider.
func TestParseToolReleaseReadsTheTaggedTriple(t *testing.T) {
	t.Parallel()
...
func TestProbeToolReleaseEstablishesTheRelease(t *testing.T) {
	...
	if release != "0.153.2" {
		t.Errorf("release = %q, want %q", release, "0.153.2")
	}
