...
// GEMINI'S ENVIRONMENT NEGATIVES POINT THE OTHER WAY FROM QWEN'S.
//
// qwen strips twelve keys, so the defect worth attacking is a filter that
// strips too much or matches by prefix. Gemini strips NOTHING, so the defect
...
// reproduced for gemini: this plugin has no exact-key strip for a prefix to be
...
//
// It is computed from the fixture rather than listed, because for gemini the
...
// TestAWholeEnvironmentWipeFailsAgainstTheGeminiGolden is the preservation
// bound, against the shipped plugin.
func TestAWholeEnvironmentWipeFailsAgainstTheGeminiGolden(t *testing.T) {
	const subject = "gemini/exec"
	...
	if len(diffs) == 0 {
		t.Fatalf("a gemini plugin that discards the WHOLE parent environment byte-matched %s", subject)
	}
...
// borrowedFilterSystem is the gemini plugin with QWEN'S filter bolted on.
//
...
// TestABorrowedRuntimeFilterFailsAgainstTheGeminiGolden is the bound that makes
// gemini's empty filter LOAD-BEARING rather than an absence nobody measured.
//
// Without it, env.go's whole comment — "gemini strips nothing, here is what it
...
func TestABorrowedRuntimeFilterFailsAgainstTheGeminiGolden(t *testing.T) {
	const subject = "gemini/exec"
	c := parityCaseFor(t, subject)
	g, dirs, req := prepareParityCase(t, c)
	...
	diffs := parity.ComparePlan(g, plan, dirs.Substitutions())
	if len(diffs) == 0 {
		t.Fatalf("a gemini plugin carrying the codex-family filter byte-matched %s; the goldens would then say nothing about which systems strip parent runtime state", subject)
	}
	if !paritycase.NamesField(diffs, "EnvRemoved") {
...
// keys, so a reader does not have to derive gemini's inheritance from two
// attack tests.
//
...
func TestTheEmptyFilterIsDeliberate(t *testing.T) {
	...
	for _, key := range inherited {
		...
		if !present {
			t.Errorf("the gemini child no longer inherits %q; env.go declares this filter EMPTY and it is not", key)
			continue
		}
		if got != want {
			t.Errorf("the gemini child rewrote %q from %q to %q", key, want, got)
		}
	}
}
