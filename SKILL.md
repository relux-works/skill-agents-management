...
| What did the caller ask for, and what ran? | `plan.ModelIdentity` → `{Requested, Launched}`; they differ when the row declared `AliasOf` (today: `muse-spark` → `muse-spark-1.3-contributor`, `astra` → `gpt-6-astra`, `sol` → `gpt-6-sol`, `luna` → `gpt-6-luna`, `opus` → `claude-opus-5-5`, `gemini-flash` → `gemini-3.8-flash-high`) |
