...
MUTATIONS = [
    ...
    Mutation(
        "vendor-alias-dangling-target",
        "pkg/vendorplugin/vendor.go",
        "if !targetDeclared {",
        'if !targetDeclared && model.AliasOf == model.ID {',
        "./pkg/vendorplugin",
        "TestTheAliasGateRefusesWhatItMustReject",
    ),
    Mutation(
        "vendor-alias-chain",
        "pkg/vendorplugin/vendor.go",
        "if target.AliasOf != \"\" {",
        "if target.AliasOf != \"\" && target.AliasOf == model.ID {",
        "./pkg/vendorplugin",
        "TestTheAliasGateRefusesWhatItMustReject",
    ),
    Mutation(
        "vendor-alias-effort-mirror",
        "pkg/vendorplugin/vendor.go",
        "if !model.Effort.SameAxis(target.Effort) {",
        "if model.Effort.Support != target.Effort.Support {",
        "./pkg/vendorplugin",
        "TestTheAliasGateRefusesWhatItMustReject",
    ),
    Mutation(
        "vendor-alias-systems-mirror",
        "pkg/vendorplugin/vendor.go",
        "if !sameSystems(model.Systems, target.Systems) {",
        "if !sameSystems(model.Systems, target.Systems) && len(model.Systems) == len(target.Systems) {",
        "./pkg/vendorplugin",
        "TestTheAliasGateRefusesWhatItMustReject",
    ),
    Mutation(
