...
| A Pi Process-A result | `pi.ValidateTurnResult(pi.TurnResultInput{Stdout: boundedStdout, Exit: actualExit, Intervention: recordedIntervention, Cleanup: cleanupOutcome})` | parse raw Pi JSONL, accept unknown schema fields/codes, infer a missing refusal document, or supervise Process B |
