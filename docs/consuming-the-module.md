...
is a short spelling of, empty when the row is its own identity. Five rows carry
it today: `muse-spark` → `muse-spark-1.3-contributor`, `astra` →
`gpt-6-astra`, `sol` → `gpt-6-sol`, `luna` → `gpt-6-luna` and `opus` →
`claude-opus-5-5`. A consumer does
...
For a non-dry-run engine-bound runtime, trusted assembly must instead install
the consumer-owned read-only adapter at construction:

```go
registry, err := vendorplugin.NewRegistryWithEngineObservationAdapters(
    systems,
    agentsInfraEngineAdapter,
)
```

The adapter declaration is read once and cannot be replaced later. Its
`ObserveEngine` input contains the graph-resolved engine plus authoritative
runtime, model and vendor-resolved profile. Return only the versioned sanitized
timestamps and `[]inferenceengine.Reading`; lifecycle handles, raw status,
credentials and process operations do not belong on this API. A registry built
with `NewRegistry` still supports zero-engine and dry-run launches, but refuses
a live engine-bound launch with `ErrEngineObservationAdapterMissing`.
...
| A Pi Process-A result | `pi.ValidateTurnResult(pi.TurnResultInput{Stdout: boundedStdout, Exit: actualExit, Intervention: recordedIntervention, Cleanup: cleanupOutcome})` | parse raw Pi JSONL, accept unknown schema fields/codes, accept a lifecycle-integrity refusal that carries detail members or a non-1 exit, infer a missing refusal document, or supervise Process B |
