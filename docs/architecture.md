...
The observation plane has one production owner. Trusted consumer assembly uses
`vendorplugin.NewRegistryWithEngineObservationAdapters`; each adapter declares
exactly one inference-engine ref, implementation kind and contract version.
Construction rejects nil/typed-nil, malformed, duplicate and unknown-version
declarations atomically, reads each declaration exactly once, and stores a
copied immutable identity snapshot. `NewRegistry` remains source-compatible
and installs no positive adapter.

For a non-dry-run engine launch, `BuildLaunch` resolves graph/model/effort,
checks caller cancellation, asks the vendor to construct the authoritative
launch/profile as a pure value, then invokes the registered adapter once under
a cooperative 10-second bound. Exact response version, query identity,
freshness and copied closed readings must validate before system `Preflight` or
plan materialization. Missing evidence fails closed. There is no concrete
engine, runtime, model or profile branch in generic core, and no adapter enters
`SpawnRequest` or `BuildLaunch`.
